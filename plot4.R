...
##Need to combined four plots into one overall graph
par(mfrow=c(2,2))

#1
g_active_power <- as.numeric(data2$Global_active_power)
plot(datetime, g_active_power, type='l', xlab = "", ylab = "Global Active Power")
...
#4
g_reactive_power <- as.numeric(data2$Global_reactive_power)
plot(datetime, g_reactive_power, xlab = "", ylab = "Global_reactive_Power", type='l')
dev.off()
